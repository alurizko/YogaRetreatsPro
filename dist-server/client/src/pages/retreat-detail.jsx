...
export default function RetreatDetail() {
    ...
    var _a = useQuery({
        queryKey: ["/api/retreats/".concat(id)],
        enabled: !!id,
    }), retreat = _a.data, isLoading = _a.isLoading;
    if (isLoading) {
    ...
    var duration = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
    return (<div className="min-h-screen bg-soft-white">
      {/* Hero Image */}
      <div className="relative h-64 md:h-96">
        <img src={retreat.imageUrl || "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=400"} alt={retreat.title} className="w-full h-full object-cover"/>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        ].map(function (item, index) { return (<div key={index} className="flex justify-between py-2 border-b border-gray-100 last:border-b-0">
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-50 rounded-xl p-6 sticky top-4">
              ...
              {availableSpots > 0 ? (<div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-6">
                ...
                </div>) : (<div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-6">
                  <span className="text-sm text-red-700">Нет доступных мест</span>
                </div>)}

              {isAuthenticated ? (availableSpots > 0 ? (<BookingForm retreat={retreat}/>) : (<Button disabled className="w-full">
                    Места закончились
